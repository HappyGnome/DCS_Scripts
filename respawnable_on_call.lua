...
respawnable_on_call={} -- namespace/ global instance for this script
...
--]]
respawnable_on_call.groupList={} 

--[[
List of group to poll to check they're still on-call
--]]
respawnable_on_call.pollList={} 

respawnable_on_call.log_i=mist.Logger:new("respawnable_on_call.log_i","info")
...
coalitionName == "red", "blue", or "all" (anything else counts as "all")
	-> coalition name that can spawn group and receive updates about them
--]]
respawnable_on_call.addGroup=function(groupName, spawnDelay, delayWhenIdle, delayWhenDead, coalitionName)
	
	local coa=respawnable_on_call.stringToSide(coalitionName)
	...
	respawnable_on_call.groupList[groupName]=newGroup
	...
	if coa then --coalition specific addition
		...
		missionCommands.addCommandForCoalition(coa,groupName,respawnable_on_call[subMenuName],
			respawnable_on_call.handleSpawnRequest_,groupName)
	else --add for all
		...
		missionCommands.addCommand(groupName,respawnable_on_call[subMenuName],
			respawnable_on_call.handleSpawnRequest_,groupName)
	end
...
--]]
respawnable_on_call.handleSpawnRequest_ = function(groupName)
	local groupInfo=respawnable_on_call.groupList[groupName]
	...
	if groupInfo then
		...
		if cRA==true or (cRA_isnum and cRA<now) then
			...
			mist.scheduleFunction(respawnable_on_call.doReSpawn_,{groupName},now+groupInfo.spawnDelay)
			
			respawnable_on_call.messageForCoalitionOrAll(groupInfo.side,
				string.format("%s will be on-call in %ds",groupName,groupInfo.spawnDelay),5)
				
			respawnable_on_call.log_i:info(groupName.." was called in.")
		else
			respawnable_on_call.messageForCoalitionOrAll(groupInfo.side,
...
--]]
respawnable_on_call.doReSpawn_ = function(groupName)
	...
	respawnable_on_call.pollList[groupName]=true
end
...
--]]
respawnable_on_call.doPoll_=function()

	local now=timer.getTime()
	for groupName,v in pairs(respawnable_on_call.pollList) do
		if v==true then --groupName names a group that should be active
			...
			if isDead then
				respawnable_on_call.pollList[groupName]=false
				respawnable_on_call.groupList[groupName]["canRequestAt"]
					= now + respawnable_on_call.groupList[groupName]["delayWhenDead"]
					
				--trigger.action.outText("Detected that asset "..groupName.." is dead",5)--DEBUG
				respawnable_on_call.log_i:info(groupName.." was detected dead.")
				
			elseif not isActive then
					respawnable_on_call.pollList[groupName]=false
					respawnable_on_call.groupList[groupName]["canRequestAt"]
						= now + respawnable_on_call.groupList[groupName]["delayWhenIdle"]
					--trigger.action.outText("Detected that asset "..groupName.." is idle",5)--DEBUG
					
					respawnable_on_call.log_i:info(groupName.." was detected idle.")
			end
		end
	end

	--schedule next poll
	mist.scheduleFunction(respawnable_on_call.doPoll_,nil,now+60)
end
...
--]]
respawnable_on_call.stringToSide = function(name)
...
--]]
respawnable_on_call.messageForCoalitionOrAll = function(side,message,delay)
	if side then
...
--start poll
mist.scheduleFunction(respawnable_on_call.doPoll_,nil,timer.getTime()+60)